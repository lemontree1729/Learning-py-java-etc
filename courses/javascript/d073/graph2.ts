...
function drawGraph(surface: HTMLElement, graph: Function, startx = 0, starty = 0) {
    surface.innerHTML = ""
...
function moveGraph(surface: HTMLElement, graph: Function, dx = 0, dy = 0) {
    while (dx-- > 0)
        surface.removeChild(surface.firstChild!)
    let x = surface.childNodes.length
    surface.childNodes.forEach(v => movePoint(<HTMLDivElement>v, dx, dy))
    // while (x < maxwidth)
