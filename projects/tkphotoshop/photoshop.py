...
def funcSave():
    ...
    if last is None:
        return
    saveFp = tkfd.asksaveasfile(defaultextension=".png", filetypes=(("PNG files", ".png"), ("All", "*.*")),)
    if saveFp is None:
        return
...
def funcReturn():
    ...
    displayImage()
    if len(temp) == 0:
        fileMenu.entryconfig("return(Ctrl+z)", state="disabled")
    print(setting)
...
def tempSave(last):
    ...
    if len(temp) < 10:
        ...
        temp.popleft()
        temp.append(last.clone())
    fileMenu.entryconfig("return(Ctrl+z)", state="normal")
        ...
        if type(value[0]) == tuple:
            ...
            if len(value) > 1:
                ...
                if len(value) == 2:
                    ...
                    if type(value[1]) == str:
                        data[key] = eval(str(value[2]) + " " + value[1] + " " + str(data[key]))
                    else:
...
try:
    ...
    tkms.showerror(
        "ModuleNotFoundError", "wand is not installed. To install, type 'pip install wand` in cmd",
    )
    funcExit()
except ImportError:
...
fileMenu.add_command(label="save file(Ctrl+s)", command=funcSave)
fileMenu.add_command(label="return(Ctrl+z)", command=funcReturn, state="disabled")
